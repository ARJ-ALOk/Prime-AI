...
AUTOMATION_ENABLED = os.getenv("JARVIS_ALLOW_AUTOMATION", "0").strip().lower() in {"1", "true", "yes"}
...
def _block_automation(ui: JarvisUI):
    msg = "Automation is disabled. Set JARVIS_ALLOW_AUTOMATION=1 to enable app control."
    ...
    while True:
        ...
        if intent == "send_message":
            ...
            if not AUTOMATION_ENABLED:
                _block_automation(ui)
                temp_memory.clear_pending_intent()
            else:
                # send_message handles missing params by asking follow-up questions.
                threading.Thread(
                    target=send_message,
                    kwargs={
            ...
            if query:
                if not SERPAPI_API_KEY:
                    msg = "Web search is disabled. Set SERPAPI_API_KEY to enable it."
                    ui.write_log(f"AI: {msg}")
                    edge_speak(msg, ui)
                else:
                    threading.Thread(
